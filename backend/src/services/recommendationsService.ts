...
const TRANSITION_RULES: Record<TransitionAction, TransitionRule> = {
  APPROVE: {
...
const recommendations: Recommendation[] = [...seedRecommendations];
...
export function listRecommendations(filters: RecommendationFilters): {
  data: Omit<Recommendation, 'auditTrail'>[];
  ...
  let results = recommendations.filter((r) => {
    ...
    if (portfolioId && r.portfolioId !== portfolioId) return false;
    if (action && r.recommendedAction !== action) return false;
    return true;
  });
...
export function transitionRecommendation(
  ...
  if (!rule) {
    ...
    (err as NodeJS.ErrnoException).code = 'INVALID_ACTION';
    throw err;
  }
  ...
  if (!rule.allowedRoles.includes(input.performedBy.role)) {
    const err = new Error(
      `Role ${input.performedBy.role} cannot perform action ${input.action}`,
    );
  ...
  if (!rule.from.includes(rec.status)) {
    const err = new Error(
      `Cannot perform ${input.action} on recommendation in status ${rec.status}. ` +
