 ...
 */
const FUZZY_PATTERNS: Array<{ pattern: string; bucket: AllocationBucket }> = [
 ...
 */
export function getAllocationBucket(category: string): AllocationBucket {
 ...
 */
export function isLockInCategory(category: string): boolean {
...
export function getCategoryGroup(category: string): string {
  ...
  if (bucket === 'debt') return 'Fixed Income';
  if (bucket === 'hybrid') return 'Hybrid';
  if (bucket === 'international') return 'Equity';
  return 'Equity';
}
