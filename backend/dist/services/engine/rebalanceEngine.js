...
exports.evaluateSellRules = evaluateSellRules;
exports.evaluateTrimHoldRules = evaluateTrimHoldRules;
exports.generateActionPlan = generateActionPlan;
const decimal_js_1 = __importDefault(require("decimal.js"));
const mandateProfiles_1 = require("./mandateProfiles");
const categoryMapper_1 = require("./categoryMapper");
// ─── Tax rate constants ───────────────────────────────────────────────────────
const STCG_RATE = new decimal_js_1.default('0.125'); // 12.5%
const LTCG_RATE = new decimal_js_1.default('0.125'); // 12.5%
// ─── Rule thresholds ─────────────────────────────────────────────────────────
const THREE_STAR_CAP = new decimal_js_1.default('0.05'); // R3: 3★ funds capped at 5% of portfolio
// ─────────────────────────────────────────────────────────────────────────────
...
function calculateCurrentAllocation(holdings) {
    const buckets = {
        large_cap: new decimal_js_1.default(0),
        mid_cap: new decimal_js_1.default(0),
        small_cap: new decimal_js_1.default(0),
        gold: new decimal_js_1.default(0),
        debt: new decimal_js_1.default(0),
        thematic: new decimal_js_1.default(0),
        international: new decimal_js_1.default(0),
        hybrid: new decimal_js_1.default(0),
    };
    let total = new decimal_js_1.default(0);
    for (const h of holdings) {
        const value = new decimal_js_1.default(h.current_value.toString());
        const bucket = (0, categoryMapper_1.getAllocationBucket)(h.category);
        buckets[bucket] = buckets[bucket].plus(value);
        total = total.plus(value);
    }
    // Build percentage map — guard against empty portfolio (division by zero)
    const pct = (val) => total.isZero() ? 0 : val.dividedBy(total).times(100).toDecimalPlaces(4).toNumber();
    const percentages = {
 ...
 */
function evaluateSellRules(holdings, totalPortfolioValue) {
    const sellDecisions = new Map();
    ...
    for (const h of holdings) {
        if (!(0, categoryMapper_1.isLockInCategory)(h.category))
            continue;
        const freeUnits = new decimal_js_1.default(h.units_free_from_exit_load.toString());
        const availUnits = new decimal_js_1.default(h.available_units.toString());
        if (freeUnits.isZero() || availUnits.isZero())
            continue;
        // Proportion of the holding that is freely redeemable
        const freePct = freeUnits.dividedBy(availUnits);
        const sellAmount = new decimal_js_1.default(h.current_value.toString()).times(freePct);
        ...
        sellDecisions.set(h.isin, {
            ...
            is_full_sell: freePct.gte(new decimal_js_1.default('0.9999')), // treat ≥99.99% as full sell
        });
    }
    ...
    const highRatedCategories = new Set();
    for (const h of holdings) {
        if (h.rating >= 4) {
    ...
    for (const h of holdings) {
        if (h.rating !== 3)
            continue;
        if (sellDecisions.has(h.isin))
            continue; // already being handled by R_LOCKIN_FREE_SELL
        const categoryKey = h.category.toLowerCase().trim();
        if (!highRatedCategories.has(categoryKey))
            continue;
        // Verify the higher-rated fund in same category is not the fund itself
        const higherRatedInSameCat = holdings.some((other) => other.isin !== h.isin &&
            other.category.toLowerCase().trim() === categoryKey &&
            other.rating >= 4);
        if (!higherRatedInSameCat)
            continue;
        sellDecisions.set(h.isin, {
            isin: h.isin,
            sell_amount: new decimal_js_1.default(h.current_value.toString()),
 ...
 */
function evaluateTrimHoldRules(holdings, existingSellDecisions, totalPortfolioValue) {
    const trimDecisions = new Map();
    for (const h of holdings) {
        // Only applies to 3★ funds not already being sold
        if (h.rating !== 3)
            continue;
        if (existingSellDecisions.has(h.isin))
            continue;
        const currentValue = new decimal_js_1.default(h.current_value.toString());
        const currentWeight = currentValue.dividedBy(totalPortfolioValue);
        if (currentWeight.lte(THREE_STAR_CAP))
            continue; // below cap → HOLD as-is
        ...
        const actualTrimAmount = decimal_js_1.default.min(idealTrimAmount, maxSellableAmount);
        if (actualTrimAmount.isZero())
            continue; // nothing sellable → pure HOLD
 ...
 */
function generateActionPlan(holdings, mandate, fundUniverse = mandateProfiles_1.DEFAULT_FUND_UNIVERSE) {
    // ── Step 1: Portfolio totals ──────────────────────────────────────────────
    const totalPortfolioValue = holdings.reduce((sum, h) => sum.plus(new decimal_js_1.default(h.current_value.toString())), new decimal_js_1.default(0));
    ...
    const trimDecisions = evaluateTrimHoldRules(holdings, sellDecisions, totalPortfolioValue);
    // Merge all sell/trim decisions into one map
    const allSellDecisions = new Map([
        ...sellDecisions,
        ...trimDecisions,
    ]);
    // ── Step 4: Build recommendation items for existing holdings ─────────────
    const recommendations = [];
    let totalNetCash = new decimal_js_1.default(0);
    for (const h of holdings) {
        const currentValue = new decimal_js_1.default(h.current_value.toString());
        ...
        const availUnits = new decimal_js_1.default(h.available_units.toString());
        const freeUnits = new decimal_js_1.default(h.units_free_from_exit_load.toString());
        const exitLoadUnits = new decimal_js_1.default(h.units_under_exit_load.toString());
        const stcgUnits = new decimal_js_1.default(h.short_term_units.toString());
        ...
        if (decision !== undefined) {
            ...
            const stGains = new decimal_js_1.default(h.short_term_gains.toString());
            const ltGains = new decimal_js_1.default(h.long_term_gains.toString());
            ...
            const exitLoadAmt = new decimal_js_1.default(h.exit_load_amount.toString())
                .times(sellPct)
                .toDecimalPlaces(2);
            ...
            const action = isTrim && !isFullSell ? 'TRIM/HOLD' : 'SELL';
            ...
            recommendations.push({
                ...
                category: h.category,
                category_group: (0, categoryMapper_1.getCategoryGroup)(h.category),
                rating: h.rating,
                rating_star: (0, categoryMapper_1.formatStarRating)(h.rating),
                ...
                flag_lockin_category: (0, categoryMapper_1.isLockInCategory)(h.category),
    ...
    const mandateTarget = mandateProfiles_1.RISK_MANDATES[mandate];
    // Post-sell portfolio value (after subtracting all sells from holdings)
    const postSellValue = totalPortfolioValue.minus([...allSellDecisions.values()].reduce((sum, d) => sum.plus(d.sell_amount), new decimal_js_1.default(0)));
    ...
    const postSellBuckets = {};
    for (const rec of recommendations) {
        const bucket = (0, categoryMapper_1.getAllocationBucket)(rec.category);
        const val = new decimal_js_1.default(rec.final_value.toString());
        postSellBuckets[bucket] = (postSellBuckets[bucket] ?? new decimal_js_1.default(0)).plus(val);
    }
    ...
    const gaps = [];
    for (const [bucket, targetVal] of Object.entries(targetValues)) {
        const currentVal = postSellBuckets[bucket] ?? new decimal_js_1.default(0);
        const gap = targetVal.minus(currentVal);
        if (gap.gt(0)) {
            gaps.push({ bucket, gap });
        }
    }
    const totalGap = gaps.reduce((sum, g) => sum.plus(g.gap), new decimal_js_1.default(0));
    for (const { bucket, gap } of gaps) {
        if (remainingCash.isZero() || totalGap.isZero())
            break;
        ...
        if (!candidate)
            continue;
        const buyRuleId = mandateProfiles_1.BUCKET_TO_BUY_RULE[bucket];
        if (!buyRuleId)
            continue;
        ...
        recommendations.push({
            ...
            rating_star: (0, categoryMapper_1.formatStarRating)(candidate.rating),
            value_now: 0,
            sold_amount: 0,
...
// ─────────────────────────────────────────────────────────────────────────────
function buildSellComment(_h, action, ruleId, ctx) {
    ...
    const holdNote = action === 'TRIM/HOLD'
        ...
        : '';
    return (`${verb}: ${ruleDesc[ruleId]}${holdNote} ` +
...
function buildHoldComment(h, ctx) {
    const wt = ctx.portfolioWeight.toDecimalPlaces(2).toNumber();
    const base = `Held (no trade). Rating=${(0, categoryMapper_1.formatStarRating)(h.rating)}. Category=${h.category}.`;
    let reason = '';
    if (h.rating >= 4) {
        reason = ` Why held: Rating is 4★/5★ (not sold unless cost-free + rules demand it).`;
    }
    else if (h.rating === 3) {
        const hasHigherInCat = ctx.holdings.some((o) => o.isin !== h.isin &&
...
function buildBuyComment(candidate, ruleId, pctOfCash) {
    const bucketLabel = {
    ...
    return (`New buy to rebalance toward mandates. ` +
        `Selection reason: New buy: 5★ ${label} Rank ${candidate.rank_in_category}. ` +
        `Deployed ${pctOfCash.toFixed(2)}% of available sell cash.`);
}
function buildSummary(recommendations, totalPortfolioValue) {
    let totalSell = new decimal_js_1.default(0);
    let totalBuy = new decimal_js_1.default(0);
    let totalExitLoad = new decimal_js_1.default(0);
    let totalTax = new decimal_js_1.default(0);
    let netCash = new decimal_js_1.default(0);
    let sellCount = 0;
    let buyCount = 0;
    let holdCount = 0;
    let trimCount = 0;
