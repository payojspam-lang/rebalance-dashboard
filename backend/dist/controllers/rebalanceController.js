...
const rebalanceEngine_1 = require("../services/engine/rebalanceEngine");
const mandateProfiles_1 = require("../services/engine/mandateProfiles");
async function rebalance(req, res, next) {
    try {
        ...
        if (!mandateProfiles_1.RISK_MANDATES[mandate]) {
            res.status(422).json({
        ...
        // Run the engine
        const { recommendations, currentAllocation, summary } = (0, rebalanceEngine_1.generateActionPlan)(holdings, mandate, fund_universe);
        // Build target allocation breakdown for the response
        const mandateProfile = mandateProfiles_1.RISK_MANDATES[mandate];
